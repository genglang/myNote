...
## Canvas
  ...
  ```
  - toDataURL()可以导出画布上绘制的图像,接受一个参数,图像的MIME类型格式
  - 浏览器会默认把图片编码为png格式,Firefox和Opera也支持JPEG编码格式
### 2D上下文
  - 2D上下文开始于左上角,基于原点进行计算
  - fillStyle和strokeStyle可以使用CSS中指定的颜色值的任何格式
  - 矩形是唯一一种可以直接在2D上下文绘制的形状,fillRect、strokeRect、clearRect都接收四个参数:矩形x、y坐标、宽、高
    - lineWidth可以控制线条宽度
    - lineCap可以控制线末端形状
  - 自定义绘制
    - arc(x,y,radius,startAngle,endAngle,conterclockwise)可以以x,y为圆心绘制一条弧线,半径为radius,起始角度和结束角度分别为startAngle、endAngle,conterclockwise表示顺时针还是逆时针
    - arcTo的参数把角度换成了终点坐标
    - bezierCurveTo(c1x,c1y,c2x,c2y,radius)从上一点开始绘制一条弧线,到(x,y)为止,并以(c1x,c1y)(c2x,c2y)为控制点
    - lineTo(x,y)从上一点绘制一条直线到(x,y)
    - moveTo(x,y)把绘制游标移动到(x,y)
    - quadrationCurveTo(cx,cy,x,y)从上一点绘制一条二次曲线,到(x,y)为止,(cx,cy)为控制点
    - rect(x,y,width,height)从(x,y)开始绘制一个矩形,绘制的是矩形路径
  - isPointInPath()方法可以接收x,y坐标,判断是否在路径上
#### 绘制文本
  - fillText和stroke可以绘制文本,接收四个参数:文本、x坐标、y坐标、可选最大像素宽度
  - 可以设置三个属性
    - font表示文本样式、字体、大小
    - textAlign表示对其方向,建议使用start、end替代left、right
    - textBaseline设置文本基线
  - 几个属性都有默认值
  - measureText()接收一个参数,即要绘制的文本,返回一个TextMetrics对象,利用font等属性计算当前指定的文本的大小
  ```
  while(context.meaureText('Hello world').width > 140) {
    fontSize --
    context.font = fontSize + 'px Arial'
  }
  ```
  - fillText和strokeText方法都可以接收第四个参数,也就是文本的最大像素宽度(并未获得所有浏览器支持)
#### 变换
  - 可以通过以下方法来修改变换矩阵
    - rotate(angle) 围绕原点旋转图像angle弧度
    - scaleX(scaleX,scaleY) 缩放图像,在x方向上乘scaleX,在y方向上乘scaleY
    - translate(x,y) 将原点坐标 移动到(x,y)执行变化之后,坐标(0,0)会变成之前由(x,y)表示的点
    - transform(m1_1,m1_2,m2_1,m2_2,dx,dy) 直接修改变换矩阵,方法是乘以下矩阵
    ```
    m1_1  m1_2  dx
    m2_1  m2_2  dy
    0     0     1
    ```
    - setTransform(m1_1,m1_2,m2_1,m2_2,dx,dy) 将变换矩阵重置为默认状态再调用transform()
    - 可以通过save()方法保存保存设置到栈中,并用restore()方法一层层读取保存栈
#### 绘制图像
  - drawImage()可以绘制图像,调用该方法的时候可以使用三种不同的参数组合
    - 传入HTML的<img>元素,以及绘制图像的起点的x和y坐标,最后两个参数表示绘制出来的宽度和高度
    - 可以选择把图像中的某个区域绘制在上下文中,接收9个参数:
      1. 绘制的图像
      2. 源图像x坐标
      3. 源图像y坐标
      4. 源图像宽度
      ...
      8. 目标图像宽度
      9. 目标图像高度
    - 传入另一个canvas元素作为第一个参数
  - 通过toDataURL()方法获得操作过后的图像结果(不能跨域)
#### 阴影
  - 2D上下文会根据以下几个属性的值,自动为形状或路径绘制出阴影
    - shadowColor 用CSS颜色格式表示阴影颜色,默认黑色
    - shadowOffsetX 形状或路径x轴方向的阴影偏移量,默认是0
    - shadowOffsetY 形状或路径y轴方向的阴影偏移量,默认是0
    - shadowBlur 模糊的像素数,默认为0,不模糊
    
#### 渐变
  - 通过createLinearGradient()方法创建一个线性渐变,接收四个参数,创建一个指定大小的渐变,并返回CanvasGradient实例
    - 起点x坐标
    - 起点y坐标
    - 终点x坐标
    - 终点y坐标
  - 通过addColorStop()方法来指定色标,接收两个参数
    - 色标位置(0开始1结束)
    - CSS颜色值
  - 通过createRadialGradient()方法创建一个径向渐变,接收六个参数
    - 起点圆圆心x坐标
    - 起点圆圆心y坐标
    - 起点圆半径
    - 终点圆圆心x坐标
...
#### 模式
  ...
  - createPattern()可以传入两个参数<img>元素(也可以是video元素)和一个表示如何重复图像的字符串(与CSS的background-repeat相同)
  - 模式和渐变一样都是从原点开始
  - 可以设置填充样式(fillStyle)为模式对象,只表示在某个特定的区域内重复图像
#### 使用图像数据
  - 2D上下文可以通过getImageData()取得原始图像数据,接收四个参数,返回的是ImageData的实例
    - 数据画面区域的x坐标
    - 数据画面区域的y坐标
    - 区域的宽度
    - 区域的高度
  - ImageData对象都有三个属性
    - width
    - height
    - data 一个保存着每一个像素数据的数组,每一个像素用4个元素保存,分别表示红绿蓝和透明度值
  - 可以实现灰阶过滤(461)
#### 合成
  - globalAlpha 接收一个0到1之间的值,用于指定绘制的透明度
  - globalComposition 表示绘制后的图形怎么与先绘制的图形结合,属性的值是字符串
    - source-over 后绘制的在先绘制的上方
    - source-in 后绘制的与先绘制的图形重叠部分可见,两者其他部分完全透明
    - source-out 后绘制的与先绘制的图形不重叠部分可见,先绘制的图形完全透明
    - source-atop 后绘制的与先绘制的图形重叠部分可见,先绘制的图形不受影响
    - destination-over 后绘制的图形在先绘制的图形下方,只有之前透明像素下的部分可见
    - destination-in 后绘制的图形在先绘制的图形下方,两者不重叠的部分完全透明
    - destination-out 后绘制的图形擦除与先绘制的图形重叠的部分
    - destination-atop 后绘制的图形在先绘制的图形下方,两者不重叠的部分先绘制的图形会透明
    - lighter 后绘制的图形与先绘制的图形重叠部分值相加,变亮
    - copy 后绘制的图形完全替代重叠的先复制的图形
    - xor 后绘制的图形与先绘制的图形重叠部分进行"异或"操作
  - 不同浏览器支持不同
### WebGL
  - WebGL是针对canvas的3D上下文,与其他Web技术不同,WebGL并不是W3C标准,而是Khronos Group制定的
  - WebGL基于OpenGL ES 2.0
  - WebGL引入了类型化数组,已保证能满足进行复杂计算的精度(464)
  
## HTML5
### 跨文本消息传递
  - 简称XDM,指来自不同域的页面间传递消息
  - 核心是postMessage()方法,在HTML5规范中,除了XDM部分之外其他部分也会提到这个方法名,但都是为了同一目的,向另一个地方传递数据
  - 另一个地方指包含在当前页面中的<iframe>元素,或者当前弹窗页面弹出的窗口
  - postMessage接收两个参数
    - 一条消息
    - 消息接收方域的字符串(非常重要,防止发送到不安全的地方)
  - 接收到XDM消息之后会触发window对象的message事件,获取到的对象有以下参数
    - data postMessage()的第一个参数
    - origin 发送消息所在的域
    - source 发送消息的文档的window对象代理,主要用于在发送上一条消息的窗口调用postMessage()方法,如果消息窗口来自同一个域,对象等于window
  - onMessage事件监听中可以判断消息来源是否来自已知页面
  - 不要通过source访问window对象的其他信息,只用来调用postMessage()方法
  - postMessage()方法第一个参数最好传字符串,因为并不是所有浏览器都支持所有数据结构
### 原生拖放
  - IE4最早提供了JS拖拽功能,但是只能拖拽图像和某些文本,之后HTML5正式将拖拽设为了正式规范
  - 通过拖放事件可以控制拖放相关的各个方面
    - dragstart 开始拖拽,光标变成不能放的符号
    - dra 拖拽中,dragstart之后持续触发
    - dragend 结束拖拽
  - 当某个元素被拖动到一个有效的放置位置时,会依次触发事件
    1. dragenter
    2. dragover 持续触发
    3. dragleave 或 drop
  - 部分元素默认不允许放置,如果拖动元素经过不允许放置的元素,就不会触发drop事件
  - 如果需要把任何元素变成有效的放置目标则需要重写dragenter和dropover方法
  - IE5引入了dataTransfer对象,是事件对象的一个属性,包含两个主要方法
    - getData() 取得setData()保存的值
    - setData() 保存值
  - dataTransfer有两个属性dropEffect和effectAllowed
    - dropEffect 可以知道拖拽元素执行哪种放置行为,可能有以下四种可能的值,必须搭配effectAllowed属性才有用
      - none 不能把拖动元素放在这,文本框意外所有元素的默认值
      - move 应该把拖动元素移动到放置目标
      - copy 应该吧拖拽元素复制到放置目标
      - link 表示放置目标会打开拖动的元素(拖动的元素必须是一个链接)
    - effectAllowed表示允许拖动元素使用哪种dropEffect,必须在ondragstart中设置
      - uninitialized 没有给被拖动的元素设置任何放置行为
      - none 被拖动的元素不能有任何行为
      - copy 只允许copy的dropEffect
      - link 只允许link的dropEffect
      - move 只允许move的dropEffect
      - copyLink 只允许copyLink的dropEffect
      - copyMove 只允许copyMove的dropEffect
      - linkMove 只允许linkMove的dropEffect
      - all 允许所有dropEffect
  - HTML5为所有HTML元素规定了一个draggable属性,表示元素是否可拖动,图像和链接都是默认可拖动
  - HTML5规范规定dataTransfer对象还应该包含下列方法和属性
    - addElement(element) 为拖动操作添加一个元素,添加这个元素只影响数据,不会影响拖动时页面元素的外观
    - claerData(format) 清除特定格式保存的数据
    - setDragImage(element,x,y) 指定一副图像,拖动发生时,显示在光标下方,xy是图像中的坐标
    - types 当前保存的数据类型,这个一个类数组的集合,以"text"这样字符串的方式保存着数据类型
### 媒体元素
  - audio和video使用时必须包含src属性
  - 可以设置width和height属性指定播放器的大小
  - poster属性指定资源加载时显示的图像
  - controls表示是否显示默认的UI控件
  - 标签中间的正文表示浏览器不支持这两个媒体元素时显示的文字
  - 内嵌source元素可以指定多个不同的媒体来源(此时媒体标签中不必设置src属性)
  - 属性(487)
  - 事件(488)
#### 自定义媒体播放器
  - 使用媒体元素的play()和pause()方法可以手动控制媒体文件的播放暂停
  - 配合事件自定义播放器
#### 检测编码器的支持情况
  - 媒体元素的canPlayType可以检测是否支持某一格式/编码器字符串,返回'probably'、'maybe'或''
  - 如果传入了一种MIME值,返回的很可能是'maybe'或'',这是因为媒体文件本身只不过是音频视频的一个容器,真正决定文件是否可播放的还是编码格式
  - 处理(490)
### 历史状态管理
  - 历史状态监控首选hashchange事件,HTML5通过更新history对象为管理历史状态提供了方便
  - 通过hashchange事件可以知道URL参数什么时候发生了变化,通过状态管理API,能够在不加载新页面的情况下改变浏览器URL
  - history.pushState()接收三个参数,状态对象,新状态的标题,可选的相对URL
  ```
  history.pushState({name:'Nicholas','Nicholas page','nicholas.html'})
  ```
  - 执行pushState()后新的状态信息会被加入历史状态栈,而浏览器地址栏也会变成新的相对URL,但是浏览器并不会真的向服务器发送请求,不会改变location.href
  - 第一个参数尽可能提供初始化页面状态所需的各种信息
  - 第二个参数目前没有浏览器实现传入空字符串(待定)
  - 点击后退会触发popstate事件,popstate事件对象有一个state属性,包含pushState的第一个参数
    - 第一次加载时state为空
    - 得到这个状态对象后,必须把页面重置为状态对象中数据表示的状态(浏览器不会自动做这些),因此返回浏览器加载的第一个页面时,event.state等于null
  - 可以通过replaceState()更新当前参数,参数与pushState()前两个参数相同,调用这个方法不会在历史状态栈中创建新状态,只会重写状态
## 错误处理与调试
### 浏览器报告的错误
  - 主流浏览器都具有某种向用户报告JavaScript错误的机制
  - 只有IE会在页面中提示错误
### 错误处理
  - ECMA-262第三版引入了try-catch,try中任何代码出现错误,就会立即退出代码执行过程,然后接着执行catch块(会接收到一个错误信息对象)
    - name 错误类型
    - message 保存着错误信息 
  - finally无论代码如何都会执行,IE7不写catch无法触发finally
  - 错误类型
    1. Error 
       - 是基类型,其他错误类型都是继承自该类型,因此所有错误类型都共享了一组相同的属性,Error类型很少见一般是开发人员抛出的自定义错误
    2. EvalError 
       - 没有把eval当成函数调用就会抛出错误,不推荐使用
    3. RangeError
       - 超出数值相应范围的时候触发
    4. ReferenceError
       - 访问不存在的变量
    ...
    6. TypeError
       - 类型错误
    7. URIError
       - encodeURI()和decodeURI()调用时参数格式不对时触发
  - 通过instanceof判断错误类型
  - 使用第三方的库时尽量进行错误处理
  - 使用throw操作符可以随时抛出自定义错误,抛出的值没有要求
  - 遇到throw值时,代码会立即停止执行,仅当try-catch语句捕获到被抛出的值时,代码才会继续执行
  - 通过new一个错误对象模拟浏览器的报错
### 抛出错误的时机
