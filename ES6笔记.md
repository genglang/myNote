...
### 私有方法和私有属性
  ...
  - 由于#是属性名的一部分,使用时必须带有#一起使用,所以#x和x是两个不同的属性
  - 之所以引入新的#符号而不是private是因为JS是动态语言,没有类型声明(Ruby使用@表示私有变量)
  - `#`不仅能写私有属性还能写私有方法
  - 私有属性也能设置getter和setter
  - 私有属性不限于从this引用,只要是在类的内部,实例也可以引用私有属性
### new.target属性
  - ES6为new命令引入了一个new.target属性,该属性一般用在构造函数中,返回new命令作用的那个构造函数
  - 如果构造函数不是new调用的,new.target就会返回undefined
  - Class内部调用new.target返回当前Class
  - 子类继承父类调用返回子类

## Class的继承
  - Class可以通过extends关键字实现继承,这比ES5的通过修改原型链实现继承要清晰和方便很多
  - super可以用来调用父类的构造函数,新建父类的this对象
  - 子类必须在constructor方法中调用super方法,否则新建实例的时候会报错
  - 子类自己的this对象,必须先通过父类的构造函数完成塑造,得到父类同样的实例属性和方法,然后进行加工加上自己的实例属性和方法
  - 如果不调用super方法,子类就获取不到this对象
  - ES5继承实质是先创造子类实例对象的this,然后再将父类的方法添加到this上
  - ES6继承实质是先将父类实例对象的属性和方法加到this上,再用子类构造函数修改this
