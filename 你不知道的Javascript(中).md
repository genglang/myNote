...
#### 大括号
   ...
   ```
   - 如果JavaScript有goto语句,理论上我们可以使用goto foo跳转到foo处执行
   - continue和break语句都可以带一个标签,因此能够像goto那样进行跳转
   - 可以实现从内层循环跳转到外层循环
   - 不建议使用带标签的循环/代码块
   - JSON字符串和对象有很大区别(JSON键有双引号)
   - {} + [] // 0 {}是代码块 实际执行+[]
   - ES6对象解构
   - JS中没有else if其实是else(if())
